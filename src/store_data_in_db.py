...
def main():
    ...
    if not os.path.exists(args.csv_path):
        logger.error(f"Historical sales CSV file not found: {args.csv_path}")
        return 1

    try:
        # Store historical sales data in database
        logger.info(f"Storing historical sales data from {args.csv_path} in database")
        ...
        if rows_inserted > 0:
            ...
            logger.info(f"Sample of data in database (first 5 rows):")
            logger.info(f"Shape: {sample_df.shape}")
            logger.info(f"Columns: {sample_df.columns.tolist()[:5]}...")
            logger.info(f"Date range: {sample_df.index.min()} to {sample_df.index.max()}")
        else:
            logger.warning("No rows were inserted into the historical_sales table")
        ...
        if not args.skip_categories:
            ...
            if not os.path.exists(args.raw_sales_path):
                logger.error(f"Raw sales CSV file not found: {args.raw_sales_path}")
                return 1

            logger.info(f"Populating category and product tables from {args.raw_sales_path}")
            ...
            if categories_inserted > 0 or products_inserted > 0:
                logger.info(f"Successfully inserted {categories_inserted} categories and {products_inserted} products into database")

                # Retrieve categories and products to verify
                categories_df = get_categories()
                logger.info(f"Categories in database: {len(categories_df)}")

                products_df = get_products()
                logger.info(f"Products in database: {len(products_df)}")

                # Show sample of products
                if not products_df.empty:
                    logger.info(f"Sample of products in database (first 5 rows):")
                    for _, row in products_df.head().iterrows():
                        logger.info(f"  {row['category_name']}: {row['name']}")
            else:
                logger.warning("No categories or products were inserted into the database")
